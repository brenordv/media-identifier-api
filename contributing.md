# Contributing
## Philosophy
Each class/function (aka tool) in this repo follows a few principles:

- **Do one thing well**: Each tool should have a focused purpose.
- **Reasonable defaults**: Tools should be useful out of the box for common cases.
- **Graceful handling**: Clear error messages, good UX, and cleanup on failure.
- **Composability**: Prefer small, reusable building blocks over giant “god” modules.

## Project structure
> Note: Names here are indicative; follow the patterns already present in the repo.

1. **Top-level layout**

   - `pyproject.toml` / `setup.cfg` / `requirements*.txt`  
     - Keep dependencies minimal and scoped.  
     - Don’t change core tooling (formatter, linter, type-checker) without discussion.

   - `src/`  
     - Main source tree. All importable code lives here.

   - `src/converters`
     - Tools to convert data from one format to another.

   - `src/media_identifiers`
     - Tools used to identify media items.

   - `src/models`
     - General models used by the application.

   - `src/repositories`
     - Tools related reading and writing to the database.
...
- **Root `README.md`**: Keep it updated with a high-level overview of the project, new functionalities, etc.
