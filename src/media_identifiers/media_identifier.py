    ...
    def identify_media(self, file_path: str) -> Optional[dict]:
        try:
            ...
            if not media_data:
                ...
                media_data = identify_media_with_open_ai(file_path, media_type=media_type)
            ...
            cached_data = self._cache.get_cached(search_term=title, search_prop_name="title", media_type=media_type)

            if cached_data:
                self._logger.debug(f"Found cached data for title: {title}")
                return cached_data

            tmdb_media = identify_media_with_tmdb(title, media_type)

            if tmdb_media is None:
                self._logger.warning(f"No TMDb data found for title: [{title}], and media type: [{media_type}]. Trying to identify with OpenAI.")
                openai_data = identify_media_with_open_ai(file_path, media_type)
